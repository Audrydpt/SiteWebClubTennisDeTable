...
import { Dumbbell, Users, MapPin, Clock, UserCheck, UserX } from 'lucide-react';
...
export default function TrainingCalendar({ member }: TrainingCalendarProps) {
  ...
  const loadTrainings = async () => {
    ...
    try {
      ...
      // Filtrer les entraînements futurs et non annulés
      const upcomingTrainings = data.filter((training: Training) => {
        const trainingDate = new Date(training.date);
        const today = new Date();
        return trainingDate >= today && training.statut !== 'annule';
      });
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="flex items-center gap-1 text-sm">
                        ...
                        <span>{training.participants.length}</span>
                        {training.maxParticipants && (
                    ...
                    <Button
                      ...
                      disabled={!isParticipating && Boolean(isFull)}
                    >
                      {isParticipating ? (
                        <>
