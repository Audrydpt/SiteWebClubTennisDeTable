...
export function OrderSummary({
  ...
  const totalPrice = [
    ...mousses.map((m) => m.prix),
    ...bois.map((b) => b.prix || 0),
    ...autres.map((a) => a.prix || 0),
        ...
        {totalItems === 0 ? (
          ...
          <>
                ...
                >
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-sm truncate">
                      {mousse.marque} {mousse.nom}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {mousse.epaisseur} • {mousse.couleur}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-sm">
                      {mousse.prix.toFixed(2)} €
                    </span>
                    <Button
                      variant="ghost"
                      size="icon"
            ...
            <div className="flex justify-between items-center text-lg font-bold">
              ...
              <span className="text-[#F1C40F]">{totalPrice.toFixed(2)} €</span>
            </div>

            {userCommandedTotal > 0 && (
              <div className="flex justify-between items-center text-sm text-muted-foreground border-t pt-2">
                <span>Total général</span>
                <span>{(userCommandedTotal + totalPrice).toFixed(2)} €</span>
              </div>
            )}
            ...
            <Button
              onClick={onSave}
              disabled={isLoading}
              className="w-full bg-[#F1C40F] hover:bg-[#F1C40F]/90 text-[#3A3A3A] font-semibold rounded-lg"
              size="lg"
            >
              {isLoading ? 'Enregistrement...' : 'Enregistrer ma commande'}
            </Button>
          </>
        )}
      </CardContent>
